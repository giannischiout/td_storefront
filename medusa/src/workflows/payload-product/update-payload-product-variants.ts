...
export const updatePayloadProductVariantsWorkflow = createWorkflow(
	...
	({ variant_ids }: WorkflowInput) => {
		const { data: productVariants } = useQueryGraphStep({
			...
			fields: ["id", "title", "options.*", "options.option.*", "product.payload_product.*"],
			filters: {
				id: variant_ids,
			},
			options: {
				...
				data.productVariants.forEach((variant) => {
					...
					if (existingVariantIndex >= 0) {
						...
						const updatedOptionValues = variant.options.map((option) => ({
							...
							value:
								existingVariant.option_values.find((ov: any) => ov.medusa_id === option.id)?.value ||
								option.value,
						}));
		...
		const result = when({ updateData }, (data) => data.updateData.items.length > 0).then(() => {
