...
const createVendorAdminStep = createStep(
	...
	async (adminData: CreateVendorAdminStepInput, { container }) => {
		...
		const vendorAdmin = await vendorModuleService.createVendorAdmins(adminData);
	...
	async (vendorAdminId, { container }) => {
		...
		const marketplaceModuleService: VendorModuleService = container.resolve(VENDOR_MODULE);
